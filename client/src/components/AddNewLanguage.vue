<template>
  <div class="m-3 text-left">
    <p>
      Choose language and click "Create" button to create the same template in
      another language. Once "Create" button is clicked, you'll be redirected to
      the new document.
    </p>
    <b-form-group
      class="pt-3"
    ...
    getRequest(`/api/v1/template-languages/${this.$route.params.template}`)
      ...
      .then(() => {
        this.languageOptions = this.languageOptions.filter(
          (element: any) =>
            !this.unavailableLanguageValues.includes(element.value)
        );
      });
  }
